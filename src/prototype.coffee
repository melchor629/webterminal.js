Plugin = (element, nconf, nshell, nenv, nhelp) ->
...
$.fn[pluginName] = (conf, options, opt, o) ->
    args = arguments;
...
$[pluginName] = (options) ->
    $(".console").webterminal(options.element, options.conf, options.shell, options.help, options.env)
